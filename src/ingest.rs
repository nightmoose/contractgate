...
pub async fn ingest_handler(
    ...
    if should_persist_per_event {
        ...
        // Iterate effective_results once; one increment per individual violation.
        // `serde_json::to_string` gives the snake_case label (rename_all = "snake_case").
        {
            let cid = contract_id.to_string();
            for vr in &effective_results {
                for violation in &vr.violations {
                    // Serde serialises ViolationKind as "missing_required_field" etc.
